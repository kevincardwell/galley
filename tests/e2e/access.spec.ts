...
test.describe.serial("invite-only access", () => {
  ...
  test("fresh install asks for the admin account", async ({ page }) => {
    ...
    await page.click("button[type=submit]");
    // The sample workspace is created and opened on first run.
    await expect(page).toHaveURL(/\/w\/marlow-finch-joinery$/);
    await expect(page.getByRole("heading", { level: 1, name: "Marlow & Finch Joinery" })).toBeVisible();
    await page.goto("/");
  ...
  test("admin creates two workspaces", async ({ page }) => {
    ...
    for (const name of ["Peak Physio", "Harbourside Bakery"]) {
      await page.goto("/");
  ...
  test("admin invites an editor to one workspace", async ({ page }) => {
    ...
    await page.selectOption("#inv-workspace", { label: "Peak Physio" });
