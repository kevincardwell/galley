...
export function Toolbar({ editor }: { editor: Editor }) {
  ...
  const setLink = () => {
    ...
    if (!clean || clean === "https://") chain().extendMarkRange("link").unsetLink().run();
    else chain().extendMarkRange("link").setLink({ href: clean }).run();
  };

  return (
    <div role="toolbar" aria-label="Formatting" className="flex items-center gap-px rounded-r border border-line bg-surface p-0.5 shadow-panel">
      ...
      <Sep />
      <Btn on={st.link} label="Link" onClick={setLink}>
...
function Btn({ on, label, onClick, children }: { on: boolean; label: string; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      type="button"
      title={label}
