...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="border-t border-line-2 pt-4">
      <h3 className="m-0 mb-2 text-xs font-medium text-ink-3">{title}</h3>
      {children}
    </section>
  );
}
...
function PersonBody({ person, workspaces, selfId, onClose }: { person: Person; workspaces: WorkspaceOption[]; selfId: string; onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Section title="Instance role">
        <div className="flex flex-wrap items-center gap-3">
          <Button size="sm" disabled={roleAction.pending || isSelf || deactivated} onClick={() => roleAction.run(() => setUserAdmin(person.id, !person.isAdmin))}>
      ...
      <Section title="Reset password">
        ...
        >
          ...
          <Button type="submit" size="sm" disabled={pwAction.pending || pw.length < 8}>{pwAction.pending ? "Saving…" : "Set password"}</Button>
        </form>
      ...
      <Section title={deactivated ? "Reactivate" : "Deactivate"}>
        <div className="flex flex-wrap items-center gap-3">
          <Button size="sm" disabled={stateAction.pending || isSelf} onClick={() => stateAction.run(() => (deactivated ? reactivateUser(person.id) : deactivateUser(person.id)))}>
        ...
        {confirmDelete ? (
          ...
          >
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" type="submit" variant="danger" disabled={typed !== person.name || delAction.pending}>{delAction.pending ? "Deleting…" : "Delete for good"}</Button>
            </div>
            <InlineError>{delAction.error}</InlineError>
          </form>
        ) : (
          <div className="flex flex-wrap items-center gap-3">
            <Button size="sm" variant="danger" disabled={isSelf} onClick={() => setConfirmDelete(true)}>Delete person</Button>
