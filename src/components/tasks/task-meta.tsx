...
export function TaskMeta({ task, className }: { task: TaskItem; className?: string }) {
  ...
  return (
    <div className={clsx("flex items-center gap-3 text-xs", done ? "text-ink-3" : "text-ink-2", className)}>
      {task.status === "doing" && !done && <span aria-label="Status: doing" className="rounded-full border border-accent-line px-1.5 text-[11px] font-medium text-accent">Doing</span>}
      {due.label && (
      ...
      {task.checklist.length > 0 && (
        <span className="tnum inline-flex items-center gap-1" title="Checklist" aria-label={`Checklist ${ticked} of ${task.checklist.length} done`}><ChecklistIcon size={13} />{ticked}/{task.checklist.length}</span>
      )}
      {task.attachments.length > 0 && (
        <span className="tnum inline-flex items-center gap-1" title="Attachments" aria-label={`${task.attachments.length} attachment${task.attachments.length === 1 ? "" : "s"}`}><PaperclipIcon size={13} />{task.attachments.length}</span>
      )}
      {openComments > 0 && (
        <span className="tnum inline-flex items-center gap-1" title="Open comments" aria-label={`${openComments} open comment${openComments === 1 ? "" : "s"}`}><CommentIcon size={13} />{openComments}</span>
      )}
