...
export default async function SettingsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="grid max-w-3xl gap-8 px-6 py-5">
      <section>
        <h2 className="m-0 mb-3 text-sm font-semibold">Workspace</h2>
        <SettingsForm workspace={{ id: ws.id, name: ws.name, url: ws.url, clientName: ws.clientName, accent: ws.accent, status: ws.status }} />
      </section>
      <section>
        <h2 className="m-0 mb-3 text-sm font-semibold">People</h2>
        <MembersPanel
          workspaceId={ws.id}
